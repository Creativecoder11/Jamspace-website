...
interface SlideItem { index: string; audience: string; image: string; }
...
function CornerMark({ className = "" }: { className?: string }) {
    return (
        <svg className={className} xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none" aria-hidden="true">
            <path d="M30 -5.24537e-06L30 30L5.24537e-06 30L2.62253e-06 14.9991L15.0018 14.9991L15.0018 -2.62299e-06L30 -5.24537e-06Z" fill="white" />
        </svg>
    );
}

function SlideContent({ slide, totalSlides }: { slide: SlideItem, totalSlides: number }) {
    return (
        <div className="absolute inset-0 z-10 flex flex-col justify-between p-6 md:p-10 lg:p-12">
            <div className="slide-top flex items-start justify-between">
                ...
                <span className="slide-index font-normal text-white">
                    ...
                    <span className="text-base md:text-xl">/{String(totalSlides).padStart(2, "0")}</span>
                </span>
            </div>
...
function revealText(tl: gsap.core.Timeline, slide: HTMLElement, at: number, reverse = false) {
...
export default function DesignHighlightSilde({ slides }: { slides: SlideItem[] }) {
    ...
    useGSAP(() => {
        ...
        const mobileSlideEls = mobileSlideRefs.current.filter((el): el is HTMLDivElement => el !== null);
        ...
        mm.add("(min-width: 768px) and (prefers-reduced-motion: no-preference)", () => {
            ...
            imageEls.forEach((img, i) => gsap.set(img, { scale: i === 0 ? 1 : IMAGE_ZOOM_AMOUNT, transformOrigin: "center center", force3D: true }));
            ...
            const totalUnits = slides.length * SLIDE_HOLD_DURATION + (slides.length - 1) * SLIDE_TRANSITION_DURATION;
            ...
            if (hintRef.current) tl.to(hintRef.current, { autoAlpha: 0, duration: 0.12 }, 0);
            tl.to({}, { duration: SLIDE_HOLD_DURATION }, 0);
            let cursor = SLIDE_HOLD_DURATION;
            for (let i = 1; i < slideEls.length; i++) {
                const start = cursor;
                const prevSlide = slideEls[i - 1]; const nextSlide = slideEls[i];
                ...
                if (imageEls[i - 1]) tl.to(imageEls[i - 1], { scale: IMAGE_ZOOM_AMOUNT, duration: SLIDE_TRANSITION_DURATION, ease: "power2.inOut" }, start);
                if (imageEls[i]) tl.to(imageEls[i], { scale: 1, duration: SLIDE_TRANSITION_DURATION, ease: "power2.inOut" }, start);
                revealText(tl, prevSlide, start, true); revealText(tl, nextSlide, start + 0.5);
                cursor += SLIDE_TRANSITION_DURATION;
                tl.addLabel(`slide-${i}`, cursor); tl.to({}, { duration: SLIDE_HOLD_DURATION }, cursor); cursor += SLIDE_HOLD_DURATION;
            }
        ...
        mm.add("(min-width: 768px) and (prefers-reduced-motion: reduce)", () => {
            gsap.set(slideEls, { xPercent: 100, autoAlpha: 0 }); gsap.set(slideEls[0], { xPercent: 0, autoAlpha: 1 });
            return () => { gsap.set(slideEls, { clearProps: "transform,opacity,visibility" }); };
        });
        ...
        mm.add("(max-width: 767px)", () => {
            ...
            const observer = new IntersectionObserver((entries) => {
                entries.forEach((entry) => {
                    ...
                    if (idx === -1) return;
                    if (entry.isIntersecting && entry.intersectionRatio >= 0.6) reveals[idx].play();
                    else reveals[idx].reverse();
                });
    ...
    return (
        <section ref={containerRef} className="relative">
            <div ref={pinRef} className="relative mb-25 hidden h-[calc(100svh-80px)] w-full overflow-hidden bg-black md:block">
                {slides.map((slide, index) => (
                    <div key={slide.index} ref={(el) => { slideRefs.current[index] = el; }} style={{ zIndex: index + 1 }} className="absolute inset-0 overflow-hidden will-change-transform transform-gpu">
                        <div ref={(el) => { imageRefs.current[index] = el; }} className="absolute inset-y-0 inset-x-[-14%] will-change-transform transform-gpu">
                            <Image src={slide.image} alt={`Residential design ideal for ${slide.audience.replace(/\.$/, "")}`} fill sizes="100vw" priority={index === 0} loading={index === 0 ? "eager" : "lazy"} className="object-cover" />
                        </div>
                        <div className="pointer-events-none absolute inset-0 bg-linear-to-t from-black/75 via-black/10 to-black/25" />
                        <SlideContent slide={slide} totalSlides={slides.length} />
                    </div>
                ))}
                <div ref={hintRef} className="pointer-events-none absolute bottom-10 left-1/2 z-20 -translate-x-1/2 text-sm text-white/80">Scroll to Explore ↓</div>
            </div>
            <div ref={mobileContainerRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 py-4 md:hidden">
                {slides.map((slide, index) => (
                    <div key={slide.index} ref={(el) => { mobileSlideRefs.current[index] = el; }} className="relative h-[50svh] w-[88vw] shrink-0 snap-center overflow-hidden rounded-2xl">
                        <Image src={slide.image} alt={`Residential design ideal for ${slide.audience.replace(/\.$/, "")}`} fill sizes="88vw" priority={index === 0} loading={index === 0 ? "eager" : "lazy"} className="object-cover" />
